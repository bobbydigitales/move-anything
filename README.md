...
2. Turn on your Move and make sure it's connected to the same network as the device you're using to install.

3. Run the installer:

```bash
curl -L https://raw.githubusercontent.com/bobbydigitales/move-anything/main/installer/install.sh | sh
```

The installer will guide you through the process, including SSH setup if needed. It will automatically:
* Check network connectivity
* Set up SSH access (generates keys if needed and guides you through adding them)
* Download and install Move Anything
* Optionally install "Pages of Sets" (see below)

**Note:** After Move firmware updates, you'll need to re-run the installer.

### Pages of Sets (Optional)

During installation, you'll be asked if you want to install "Pages of Sets", which gives you unlimited pages of sets on the Move. Hold Shift + Left/Right arrows to change pages. _**Warning: This will rearrange your sets and change their colors!**_ You can install this feature later by running the installer again.

## Installation (Windows)

If you're on Windows, install Git Bash (available with Git for Windows): <https://git-scm.com/downloads/win>. Once installed, launch Git Bash and run the installer command above.
